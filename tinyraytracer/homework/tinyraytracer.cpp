...
Vec3f cast_ray(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, const std::vector<Light> &lights, int i, int j, size_t depth = 0)
{
    Vec3f point, N;
    Material material;
...
#pragma omp parallel for
    ...
    { // actual rendering loop
        for (size_t i = 0; i < width; i++)
        {
            ...
            framebuffer[i + j * width] = cast_ray(Vec3f(0, 0, 0), Vec3f(dir_x, dir_y, dir_z).normalize(), spheres, lights, i, j);
        }
    }
